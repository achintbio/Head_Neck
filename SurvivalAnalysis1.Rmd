...
```{r}
surv_obj_repeat<-function(x,y){return(Surv(time = x,event=y))}

risk_table_func<-function(x){return(ggsurvtable(x,break.time.by=0.5,risk.table.type="percentage"))}

res_cox<-coxph(surv_obj~Site,data=x)
...
surv_objs2<-mapply(FUN=surv_obj_repeat,x = survival_master2[,2:4],y = survival_master2[,5:7])
...
legends.labs_list<-c(c("Treatment Interruptions(YES=1, NO=0)",c("Hypopharynx","Larynx","Oral Cavity","Nasopharynx","Oropharynx"),c("Stage I or Stage II","Stage III or Stage IV"),c("Ecog 0","Ecog 1","Ecog 2/3/4"),c("70-80","80+"),c("CCI= 5/6","CCI=7+"))
                     

names(plots_list_DS)<-c("TI","Site","stage","ecog","age","CCI")
#0.2,0.00013,<0.00001,0.00068,0.1,0.39
g<-plots_list_DS$TI + theme_survminer(legend.position=c(0.2,0.2))
reg_ageOS<-regression_output(coxph(surv_object~age_cat,data=Survival_Age))


```

The code below generates risk tables or "actuarial tables" for each of the three events we are interested in
```{r}

lcr<-lapply(suv_fits$LCRdate,risk_table_func)
OS<-lapply(suv_fits$Living_date,risk_table_func)
DS<-lapply(suv_fits$DC,risk_table_func)
